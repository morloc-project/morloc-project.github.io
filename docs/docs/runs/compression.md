...
The long form is `--compression-level`. A returned value is written uncompressed unless `-z` is given. The flag is silently a no-op for non-packet output formats (JSON, MessagePack as a raw file, voidstar, Arrow, Parquet, CSV); for those the file shape itself has no header in which to record a compression algorithm.

A stream written to `@stdout` is different: each sub-packet already carries the level its `@write` chose, so that level is the default and `-z` is an override. `./nexus -f packet stream > out.dat` keeps the `@write` levels; `./nexus -f packet -z 5 stream > out.dat` compresses every sub-packet at 5, and `-z 0` writes them all uncompressed. The pool compresses the sub-packets in both cases, so the footer’s sub-packet index describes the bytes on disk and `out.dat` is a valid `IFile` as well as an `IStream`. As with a file, two sub-packets of one stdout stream may not use different levels; under `-z` they cannot.
